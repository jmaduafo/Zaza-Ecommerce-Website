...
const NavbarCopy = async ({ setNavClick, navClick }) => {
  const [isHome, setIsHome] = useState(true)
  const [backgroundScroll, setBackgroundScroll] = useState('transparent')
  ...
  //   const categoryArray = await categoryData.categories
  //   const subcategoryArray = await subcategoryData.subcategories
  //   const titleArray = []

  //   for (let i = 0; i < subcategoryArray.length; i++) {
  //     // console.log(subcategoryArray[i].title)
  //     const currentTitle = await subcategoryArray[i].title
  //     if (!titleArray.includes(currentTitle)) {
  ...
  //   console.log("CAT", categoryArray[i].name)


  //   for (let k = 0; k < titleArray.length; k++) {
  //     console.log("TIT", titleArray[k])

  //   for (let j = 0; j < subcategoryArray.length; j++) {


  //       if  (subcategoryArray[j].title === titleArray[k]) {

  //         console.log("SUB", subcategoryArray[j].name)

  //         // if () {

  //         // categoryArray[i].name === subcategoryArray[j].category.name) &&


  //           // console.log(subcategoryArray[j].category.name, i, j)
  //           // {lingerieStyle?.map(lingerie => {
  //           //   return (<Link key={lingerie.subcategory} to={`/lingerie/${lingerie.subcategory}`}><p onClick={(e) => { setNavClick(e.target.innerText); console.log(e.target.innerText) }}>{lingerie.subcategory}</p></Link>)
  //           // })}
  //         // }
  ...
  // const matchingCategory = category.name === subcategory.category.name;
  // const additionalCondition = ...; // Define your additional condition here

  // if (matchingCategory && additionalCondition) {
                  ...
                  {subcategoryData.map(subcategory => (
                    <div>
                      ...
                      {subcategory.map(subcategory => {
                        return (<Link key={subcategory.name} to={`/lingerie/${subcategory.name}`}><p onClick={(e) => { setNavClick(e.target.innerText); console.log(e.target.innerText) }}>{subcategory.name}</p></Link>)
                      })}
                    </div>
                  ))}
